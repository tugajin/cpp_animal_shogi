...
public:
    ...
    Position next(const Move2 action) const;
    void dump() const;
    Key history() const {
...
Position::Position(const ColorPiece pieces[], const Hand hand[], const Color turn) {
   REP(i, SQ_END) {
    ...
    this->hand_[BLACK2] = hand[BLACK2];
    this->hand_[WHITE2] = hand[WHITE2];
    this->turn_ = turn;
    this->history_[this->ply_] = hash2::hash_key(*this);
}
...
std::string Position::str() const {
    ...
    ret += "後手:" + hand_str(this->hand_[WHITE2]) + "\n";
    ...
    ret += "先手:" + hand_str(this->hand_[BLACK2]) ;
    return ret;
}

bool Position::is_ok() const {
    if (this->turn() != BLACK2 && this->turn() != WHITE2) {
#if DEBUG
                Tee<<"error turn\n";
#endif
        return false;
    ...
    if (!hand_is_ok(this->hand_[BLACK2])) {
#if DEBUG
                Tee<<"error black hand\n";
#endif
        return false;
    } 
    if (!hand_is_ok(this->hand_[WHITE2])) {
#if DEBUG
...
Position Position::next(const Move2 action) const {
    ASSERT2(this->is_ok(),{
        Tee<<"prev_next\n";
    ...
    next_pos.history_[next_pos.ply_] = hash2::hash_key(next_pos);
...
void Position::dump() const {
    ...
    Tee<<"black_hand:"<<static_cast<int>(this->hand_[BLACK2])<<std::endl;
    Tee<<"white_hand:"<<static_cast<int>(this->hand_[WHITE2])<<std::endl;
}

void test_common() {
#if DEBUG
    ASSERT(change_turn(BLACK2) == WHITE2);
    ASSERT(change_turn(WHITE2) == BLACK2);
    ...
    ASSERT(color_piece(HIYOKO,BLACK2) == BLACK_HIYOKO);
    ASSERT(color_piece(KIRIN,BLACK2) == BLACK_KIRIN);
    ASSERT(color_piece(ZOU, BLACK2) == BLACK_ZOU);
    ASSERT(color_piece(LION, BLACK2) == BLACK_LION);
    ASSERT(color_piece(NIWATORI, BLACK2) == BLACK_NIWATORI);

    ASSERT(color_piece(HIYOKO, WHITE2) == WHITE_HIYOKO);
    ASSERT(color_piece(KIRIN, WHITE2) == WHITE_KIRIN);
    ASSERT(color_piece(ZOU, WHITE2) == WHITE_ZOU);
    ASSERT(color_piece(LION, WHITE2) == WHITE_LION);
    ASSERT(color_piece(NIWATORI, WHITE2) == WHITE_NIWATORI);
#endif
}
}
#endif