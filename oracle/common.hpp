...
#define REP_COLOR(i) for (auto i = BLACK2; (i) < COLOR_SIZE; ++(i)) 
...
enum Color : int {
    BLACK2 = 0, WHITE2 = 1, COLOR_SIZE = 2, COLOR_NONE = 3,
};
...
inline constexpr Move2 move(const Square from, const Square to, bool prom = false) {
    return static_cast<Move2>(
        (static_cast<int>(prom) << 12) | (from << 6) | to
    );
}

inline constexpr Move2 move(const Square to, const Piece p) {
    return static_cast<Move2>(
       (1 << 13) | (p << 6) | (to)
    );
}

inline constexpr Square move_from(const Move2 m) {
    return static_cast<Square>((m >> 6) & 0x3F);
}

inline constexpr Square move_to(const Move2 m) {
    return static_cast<Square>(m & 0x3F);
}

inline constexpr Piece move_piece(const Move2 m) {
    return static_cast<Piece>((m >> 6) & 0x1F);
}

inline constexpr bool move_is_drop(const Move2 m) {
    return (m & (1 << 13)) != 0;
}

inline constexpr bool move_is_prom(const Move2 m) {
    return (m & (1 << 12)) != 0;
}
...
inline bool move_is_ok(const Move2 m) {
    if (move_is_drop(m)) {
...
inline std::string move_str(const Move2 m) {
    std::string ret = "";
    if (move_is_drop(m)) {
...
void init_table() {
    ...
    REP(i, 12) {
        ...
        g_piece_color_piece[BLACK2][i] = g_piece_color_piece[WHITE2][i] = -1;
    }
    auto n = 0;
    ...
    g_piece_color_piece[BLACK2][HIYOKO] = BLACK_HIYOKO;
    g_piece_color_piece[BLACK2][KIRIN] = BLACK_KIRIN;
    g_piece_color_piece[BLACK2][ZOU] = BLACK_ZOU;
    g_piece_color_piece[BLACK2][LION] = BLACK_LION;
    g_piece_color_piece[BLACK2][NIWATORI] = BLACK_NIWATORI;

    g_piece_color_piece[WHITE2][HIYOKO] = WHITE_HIYOKO;
    g_piece_color_piece[WHITE2][KIRIN] = WHITE_KIRIN;
    g_piece_color_piece[WHITE2][ZOU] = WHITE_ZOU;
    g_piece_color_piece[WHITE2][LION] = WHITE_LION;
    g_piece_color_piece[WHITE2][NIWATORI] = WHITE_NIWATORI;
